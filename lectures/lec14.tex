...
\begin{prop}[Hessian and critical points]\label{prop:hess-crit-points}
	...
	\begin{enumerate}
		...
		\item $H$ indefinite $\implies$ $x_{0}$ is neither max or min and $x_{0}$ is called a \textit{saddle point}.
	\end{enumerate}
\end{prop}
\begin{proof}
...
\begin{proof}
    ...
    We write $c_{\ell}z^{\ell} = \rho e^{i\alpha } \cdot \left| z \right|^{\ell} e^{i \ell \text{arg}(z)}$ for some $\rho, \alpha \in \mathbb{R}$. If we can achieve $\alpha + \ell \cdot \text{arg}(z) = \pi$, by tweaking the value of $z$, we are done. And this can of course be done always, so once we get that, we're left with
    \begin{align*}
...
\begin{proof}
    ...
    \begin{align*}
        g(s) = f((1-s)x_{0} + sy) = f(x_{0} + sv)
    \end{align*}
    for $v = y-x$, which is a convex function of one variable. Since $g \in C^{2}([0,1])$, we get that $g'' \geq 0$. By the same computation as in the Taylor Theorem
    \begin{align*}
        g''(s) = \frac{1}{2} v \cdot Hf(x_{0} + sv) v \geq 0 \implies (s = 0 \implies \forall v \in \mathbb{R}^{n}: \frac{1}{2}v \cdot Hf(x_{0}(?))v \geq 0).
    \end{align*}
