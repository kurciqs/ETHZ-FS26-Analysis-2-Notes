...
{
	\definecolor{chalkorange}{RGB}{250, 105, 65}
		\begin{center}
		\begin{tikzpicture}[
		    scale=0.7,
		    transform shape,
		    thick,
		    black,
		    blob/.style={smooth cycle, tension=0.8, draw=black, line width=1.2pt},
		    point/.style={circle, fill=chalkorange, inner sep=0pt, minimum size=4pt}
		]
			\node[font=\huge] at (-2, 2.5) {$\mathbb{R}^n$};
			\node[font=\huge] at (3.5, 2.5) {$\mathbb{R}^m$};
			\node[font=\huge] at (8.5, 2.5) {$\mathbb{R}^k$};

			\draw[blob] plot coordinates {
				(-2.5,0.8) (-0.8,1.2) (0.2,0) (-0.2,-1.5)
				(-1.5,-2.5) (-2.8,-1.8) (-3.2,-0.5)
			};
			\node[font=\LARGE] at (-0.5, 1.6) {$U$};

			\draw[blob] plot coordinates {
				(2.2,0.2) (3,0.5) (4.5,1.2) (5.8,0.2)
				(5.5,-1.5) (4,-2) (3,-1) (2,-1)
			};
			\node[font=\LARGE] at (4.7, 1.8) {$V$};

			\node[point] (x0) at (-1.5, -0.6) {};
			\node[chalkorange, below right, font=\LARGE, yshift=-2pt] at (x0) {$x_0$};

			\node[point] (fx0) at (3.5, -0.3) {};
			\node[chalkorange, right, font=\LARGE, xshift=2pt] at (fx0) {$f(x_0)$};

			\node[point] (gfx0) at (8.8, -0.8) {};
			\node[chalkorange, above right, font=\LARGE, yshift=2pt] at (gfx0) {$g(f(x_0))$};

			\draw[->, >=stealth, line width=1.2pt]
				(-0.2, 0) to[out=40, in=140]
				node[midway, above, font=\LARGE, yshift=2pt] {$f$}
				(2.8, -0.4);

			\draw[->, >=stealth, line width=1.2pt]
				(5.2, 0.2) to[out=30, in=150]
				node[midway, above, font=\LARGE, yshift=2pt] {$g$}
				(8.1, -0.6);
		\end{tikzpicture}
	\end{center}
}
...
\begin{remark}
	...
	\end{align*}
	    {
	        \definecolor{chalkgreen}{RGB}{80, 200, 70}
	        \begin{center}
	            \[
	            \begin{tikzcd}[row sep=0.2ex, column sep=3.2em, cells={nodes={inner sep=0.2ex}}]
	                {\scriptstyle \color{chalkgreen}(x_{1},\ldots,x_{n})} &
	                {\scriptstyle \color{chalkgreen}(y_{1},\ldots,y_{m})} &
	                {} \\
	                U \arrow[r, "f"] &
	                V \arrow[r, "g"] &
	                \mathbb{R}^{k} \\
	                {\scriptstyle \subset \mathbb{R}^{n}} &
	                {\scriptstyle \subset \mathbb{R}^{m}} &
	                {}
	            \end{tikzcd}
	            \]
	        \end{center}
...
\end{theorem}
{
    \definecolor{chalkgreen}{RGB}{80, 200, 70}
    \definecolor{chalkorange}{RGB}{250, 105, 65}
    \definecolor{chalkblue}{RGB}{90, 180, 225}
	    \begin{center}
		        \begin{tikzpicture}[
		          scale=0.5,
		          transform shape,
            ...
            \node[chalkgreen, font=\Huge] at (3.5, 3.2) {$U$};

            \coordinate (X0) at (-1.2, -0.8);
            \coordinate (X0H) at (0.8, 1.2);

	            \draw[chalkorange, line width=1.5pt] (X0) -- (X0H);

            \foreach \t in {0.10,0.20,0.30,0.40,0.50,0.60,0.70,0.80,0.90}{
                \node[point, draw=chalkorange] at ($(X0)!\t!(X0H)$) {};
            }

            \node[point, draw=chalkorange] at (X0) {};
            \node[chalkorange, font=\huge, below right] at (-1.5, -0.8) {$x_0$};

            \node[point, draw=chalkgreen] at (X0H) {};
            \node[chalkgreen, font=\huge, below right] at (0.8, 1.2) {$x_0+h$};

	            \draw[->, >=stealth, chalkblue, line width=2pt]
	                (4.5, -0.5) -- (6.5, 1.5)
                node[midway, below right, font=\huge] {$h$};
        \end{tikzpicture}
    \end{center}
}
...
\end{definition}
{
    ...
    \begin{center}
        \begin{tikzpicture}[
            ...
            \node[chalkorange, font=\Huge] at (0, 1) {$A_1$};

            \coordinate (P1) at (-3.7, 1);
            \coordinate (P2) at (-0.4, 2.2);
            \draw[greenline] (P1) -- (P2);
            \node[greenpoint] at (P1) {};
            \node[greenpoint] at (P2) {};
            \coordinate (Xbad) at ($(P1)!0.45!(P2)$);
            \draw[counterexample] ($(Xbad)+(-0.18,-0.28)$) -- ($(Xbad)+(0.18,0.28)$);
            \draw[counterexample] ($(Xbad)+(-0.18,0.28)$) -- ($(Xbad)+(0.18,-0.28)$);

	            \draw[draw=chalkorange, line width=1.35pt]
            ...
            \node[chalkorange, font=\Huge] at (8.0, 2.15) {$A_2$};

            \coordinate (P3) at (4.9, 0.4);
            \coordinate (P4) at (6.0, 2.55);
            \draw[greenline] (P3) -- (P4);
            \node[greenpoint] at (P3) {};
            \node[greenpoint] at (P4) {};

            \coordinate (P5) at (5.8, -2.35);
            \coordinate (P6) at (8.7, -0.4);
            \draw[greenline] (P5) -- (P6);
            \node[greenpoint] at (P5) {};
            \node[greenpoint] at (P6) {};
        \end{tikzpicture}
    \end{center}
}
...
\begin{definition}[Gradient]\label{def:gradient}
    ...
    \begin{align*}
        \nabla f(x) = Jf(x)^\top.
    \end{align*}
    Notice that technically, $Jf(x) = (\partial_{1}f(x), \dots, \partial_{n}f(x))$ is a row vector and we want the gradient to be column, so we take the transpose.
\end{definition}
