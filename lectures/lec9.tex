...
\begin{remark}
	...
	\begin{align*}
		J(g \circ f)(x_{0}) = Jg(f(x_{0})) \cdot Jg(x_{0}).
	\end{align*}
    Component-wise, for $1 \leq i \leq n$, this becomes the $\mathbb{R}^{k}$ vector
    \begin{align*}
        \frac{\partial (g \circ f)}{\partial x_{i}}(x) = \sum_{j=1}^{m} \frac{\partial g}{\partial y_{j}} (f(x)) \cdot \frac{\partial f_{j}}{\partial x_{i}}(x) = \partial_{j}(g \circ f)(x) = \sum_{j=1}^{m} \partial_{j}g(f(x)) \cdot \partial_{i}f_{j}(x),
    \end{align*}
