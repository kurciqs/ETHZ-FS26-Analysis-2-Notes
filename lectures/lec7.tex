...
\begin{definition}[Hilbert-Schmidt norm]\label{def:hilbert-schmidt-norm}
    ...
    \begin{align*}
        \lVert M \rVert_{2} = \sqrt{ \text{tr}(M^* M)} = \sqrt{\sum_{i=1}^{n} \left| Me_{i} \right| },
    \end{align*} 
...
\begin{remark}\label{rem:global_conventions}
    From now on, $\mathbb{R}^{n}$ will be the vector space over $\mathbb{R}$ with the Euclidean norm $\left| \cdot \right|$. It is also an inner product space, with the inner product as in \ref{def:euclid_norm}. It is also a metric space with $d(x,y) = \left| x-y \right|$. 

    If we say that $x_{n} \to x$ in $\mathbb{R}^{n}$, then we mean $\left| x_{n} - x \right| \to 0$. What will also be central is that convergence is equivalent to component-wise convergence $\left| x_{i,n} - x_{i} \right| \to 0$. 

    Also if $U \subset \mathbb{R}^{n}$ is an open set and $f: U \to \mathbb{R}^{m}$ a function, we say
    \begin{align*}
        y = \lim_{x \to x_{0}} f(x)
    \end{align*}
    if and only if for every sequence $(x_{k})_{k}^{\infty}$ with $x_{k} \to x$, we have $f(x_{k}) \to y$. 
\end{remark}
...
\begin{definition}[Differential]\label{def:differential}
    Let $f: U \to \mathbb{R}^{m}$ with $U \subset \mathbb{R}^{n}$ open be a function. We say that $L : \mathbb{R}^{n} \to \mathbb{R}^{m}$, a linear map, is the differential of $f$ if for all $x_{0} \in U$
    \begin{align*}
        \lim_{x \to 0} \frac{f(x_{0}+x) - f(x) - Lx}{\left| x \right|} = 0.
    \end{align*}
\end{definition}
