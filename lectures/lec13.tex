...
\begin{lemma}[Eigenvalue minimisation]\label{lem:eig-min}
	Assume $v_{1}, \dots, v_{k} \in \mathbb{R}^{k}$ for $k \in \left\{ 1, \dots, n-1 \right\}$ are linearly independent vectors and $A$ is an $n \times n$ symmetric real matrix with eigenvectors $v_{i}$, so $A v_{i} = \mu_{i} v_{i}$ for $\mu_{i} \in \mathbb{R}$. Then $\exists w \in \mathbb{R}^{n}$ such that $\left| w \right| = 1$, $w \cdot v_{i} = 0$ and
	\begin{align*}
...
\begin{proof}
	...
	\begin{align*}
		\partial_{i}f(x) = \lim_{t \to 0} \frac{f(x + te_{i}) - f(x)}{t} & = \lim_{t \to 0} \frac{(x+te_{i}) \cdot A(x+te_{i}) - x \cdot Ax}{t}                    \\
		                                                                 ...
		                                                                 & = 2 e_{i} \cdot Ax
	\end{align*}
	and so $e_{i} \cdot \nabla f(x) = \partial_{i}f(x) = 2e_{i} \cdot Ax$. Now, since $g_{*}(x) = x \cdot Ix$, we get $\nabla g_{*}(x) = 2x$.
	And for the rest,
	\begin{align*}
	...
	\end{align*}
	which implies $\lambda_{1} = \dots = \lambda_{k} = 0$, as $(v_{1}, \dots, v_{k})$ form a basis. But then $\lambda_{0}^{2} + \lambda_{*}^{2} = 1$ and $Aw = \lambda w$ with $\lambda = -\lambda_{*} / \lambda_{0}$. And we can't have $\lambda_{0} = 0$, because that would imply $w = 0$ and we had $\left| w \right| = 1$.
\end{proof}
...
\begin{definition}[Hessian matrix]\label{def:hessian-matrix}
	...
	\begin{align*}
		...
		\end{pmatrix}
	\end{align*}
	Notice that by Schwarz Theorem \ref{thm:schwarz}, $Hf(x_{0})^\top  = Hf(x_{0})$.
\end{definition}

\begin{prop}[Necessary conditions for the Hessian]\label{prop:hess-cond}
	Let $U \subset \mathbb{R}^{n}$ be open and $f \in C^{3}(U)$. If $f$ has a local minimum at $x_{0} \in U$, then $Hf(x_{0})$ is non-negative definite, i.e. $x \cdot Hf(x_{0}) x \geq 0 \; \forall x \in \mathbb{R}^{n}$.
\end{prop}
\begin{proof}
    Set $H:=Hf(x_{0})$. Using Taylor and $\nabla f(x_{0}) = 0$ (\ref{prop:grad-vanish}), we write (check with the Taylor formula)
    \begin{align*}
        f(x_{0} + x) - f(x_{0}) = \frac{1}{2} x \cdot Hf(x_{0})x + \mathcal{O}(\left| x \right|^{3}).
    \end{align*}
    We can use the spectral theorem on $H$, giving $O^\top H O = \Lambda$ where $\Lambda$ is a diagonal matrix. If we then set $y = O x \iff x = O^\top y$, we would have
    \begin{align*}
        f(x_{0} + O^\top y) - f(x_{0}) = \frac{1}{2} y \cdot \Lambda y + \mathcal{O}(\left| y \right|^{3}).
    \end{align*}
    By minimality, $\frac{1}{2} y \cdot \Lambda y \geq 0$ once $y$ is sufficiently close to $x_{0}$, which proves that all eigenvalues of $\Lambda$ are non-negative, which one can check is equivalent to $Hf(x)$ being non-negative definite. (Will we circle back to this or is this supposed to be the proof?)
\end{proof}
