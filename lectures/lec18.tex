...
\begin{proof}
    ...
    Conversely, take $\tau \in D \Psi^{-1}_{\Psi(p)}(E)$. Because the image of $\Psi$ is open and $E$ is a subspace, we can take $t > 0$ small enough and get
    \begin{align*}
...
\begin{remark}[Tangent space is differential of parametrisation]\label{rem:parametrisation-tangent-space}
	We notice that for a parametrisation $\phi$ of $M$ around $p \in M$,
	\begin{align*}
	...
	\end{align*}
	Hence $D \phi_{y_{0}}(\mathbb{R}^{d}) \subset T_{p}M \subset \mathbb{R}^{n}$. And the map $D \phi_{y_{0}}$ has full rank $d$, and its image is a subspace of $T_{p}M$, which is $d$-dimensional. Hence they must coincide.
\end{remark}
...
\begin{definition}[Dyadic subset]\label{def:dyadic-subset}
    ...
    \end{align*}
    such that all the sets we are unioning over are disjoint (so $a_{i} \neq a_{j}$ for $i \neq j$).
\end{definition}
...
Now we can move onto measuring the volumes of general sets. It's just like the Riemann integral in Analysis 1.
...
\begin{definition}[Jordan measurable sets]\label{def:jordan-measure}
    Let $E \subset \mathbb{R}^{n}$ be bounded. $E$ is called Jodran measurable if $\mu_{\text{in}}(E) = \mu_{\text{out}}(E)$ and then we write
    \begin{align*}
