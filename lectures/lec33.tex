...
In the special (but common) case that $f(t) = q(t)e^{ \omega t }$ can be solved as follows. If $\omega$ is not a root of $p$, then consider $y(t) = Q(t) e^{ \omega t }$ for some polynomial $Q$ with degree $n$. If $\omega = \alpha_{j}$ is a root of $p$, consider instead $y(t) = Q(t)t^{m_{j}}e^{ \omega t }$.
