...
But how do we extend everything we did since introducing the $d$-volume to submanifolds that do not admit a single parametrisation? Indeed, a lot of work will have to be done to define an integral for multiple parametrisation and to further also prove that it is a sound definition. Only after that can we tackle the global version of the divergence theorem.
...
\begin{proof}
    ...
    \begin{align*}
        S(x) = \sum_{\ell=1}^{N} \xi_{\ell}(x) + \xi_{0}(x), 
    \end{align*}
