...
\begin{theorem}[Implicit function theorem]\label{thm:impl-func-theorem}
	...
	\end{align*}
	and $g$ is class $C^{k}$. So the level set of $\lambda$ is a graph of a $C^{k}$ function $g$.
...
\begin{proof}
	...
	\end{align*}
	whe\text{Re} $J_{y}f(x,g(x))$ is invertible, since it's close enough to $J_{y}f(x_{0}, y_{0})$\footnote{I know it's a little heuristic, but it's actually true, you can estimate the Hilbert-Schmidt norm from below like in the Inverse Function Theorem, left as an exercise. But most of the time more that half of this stuff doesn't even come up in computation, it's just very technical.}.
\end{proof}
