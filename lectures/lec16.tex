...
\begin{theorem}[Inverse function theorem]\label{thm:inv-func-thm}
	...
	\begin{enumerate}
		...
		\item $V = f(U_{0})$ is open and we get the inverse $g = (f|_{U_{0}})^{-1}$
...
\begin{proof}
	...
	We take $L = Df_{0}$ and define the function $F = L^{-1} \circ f$. Using the chain rule, the differential of this is
	\begin{align*}
...
\begin{definition}[Diffeomorphism]\label{def:diffeo}
	Let $U, V \subset \mathbb{R}^{n}$ be open sets and $f: U \to V$ a function that is $C^{1}$, bijective and $f^{-1}$ is $C^{1}$ as well. Then we call $f$ a diffeomorphism. $f$ is a $C^{k}$-diffeomorphism if $f$ and $f^{-1}$ are both class $C^{k}$.
\end{definition}
...
\begin{theorem}[Implicit function theorem]\label{thm:impl-func-theorem}
	...
	\begin{align*}
		Jf(x_{0}, y_{0}) = \Big(J_{x}f(x_{0}, y_{0}) \mid J_{y}f(x_{0},y_{0})\Big) \in \mathbb{R}^{(n-d) \times n},
	\end{align*}
	...
	Then $\exists U_{0} = B_{r}(x_{0}) \times B_{s}(y_{0}) \subset U$ and there is a function $g: B_{r}(x_{0}) \to B_{s}(y_{0})$ such that,
	\begin{align*}
		\left\{ (x,y) \in U_{0} \mid f(x,y) = 0 \right\} = \left\{ (x,y) \in U_{0} \mid y = g(x) \right\}
	\end{align*}
	and $g$ is class $C^{k}$. So the level set of $0$ is the graph of a $C^{k}$ function $g$.

	And finally, for all $x \in B_{r}(x_{0})$, the Jacobi of $g$ is given by
	\begin{align*}
		Jg(x) = -J_{y}f(x, g(x))^{-1} \cdot J_{x}f(x, g(x)).
	\end{align*}
\end{theorem}
\begin{proof}
	Fix $x_{0}$ and $y_{0}$ as in the statement. Define the map $\Phi: U \to \mathbb{R}^{n}$ such that for $x \in \mathbb{R}^{d}, y \in \mathbb{R}^{n-d}$,
	\begin{align*}
		\Phi(x,y) = (x, f(x,y)).
	\end{align*}
	...
	\begin{align*}
		(\xi, \eta) := \Phi(x,y) = (x, f(x,y)), \text{ which is equivalent to } (x,y) = \Psi(\xi, \eta)
	\end{align*}
	with $\xi  \in \mathbb{R}^{d}, \eta \in \mathbb{R}^{n-d}$. We see that $\xi = x$ and we can express $\Psi$ in the form
	\begin{align*}
	...
	\begin{align*}
		J_{x}f(x,g(x)) + J_{y}f(x,g(x)) \cdot Jg(x) = 0,
	\end{align*}
	where $J_{y}f(x,g(x))$ is invertible, since it's close enough to $J_{y}f(x_{0}, y_{0})$\footnote{I know it's a little heuristic, but it's actually true, you can estimate the Hilbert-Schmidt norm from below like in the Inverse Function Theorem, left as an exercise. But most of the time more that half of this stuff doesn't even come up in computation, it's just very technical.}.
\end{proof}
