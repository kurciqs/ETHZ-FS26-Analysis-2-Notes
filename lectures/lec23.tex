\begin{theorem}[Fubini]\label{thm:fubini}
	Let $A \subset \mathbb{R}^{n}$ be bounded in $(-C, C)^{n}$ and $f: A \to \mathbb{R}$ be Riemann integrable over $A$. Denote $\tilde{f} = f \cdot \boldsymbol{1}_{A}$ defined on $\mathbb{R}^{n}$. For $x \in \mathbb{R}^{k}$, denote $\tilde{f}_{x}(y) = \tilde{f}(x,y)$ and define the marginals
	\begin{align*}
		\underline{ \varphi }(x) = \underline{ \int } \tilde{f}_{x}(y) \,dy, \quad
		\overline{ \varphi }(x) = \overline{ \int } \tilde{f}_{x}(y) \, dy.
	\end{align*}
