...
\begin{proof}
	...
	\end{align*}
	As $\lambda^{m} \to 0$ with $m \to \infty$, the sequence is Cauchy and converges to $x = \lim_{n \to \infty} x_{n}$, because $X$ is complete. For this point, we thus have, using sequential continuity,
	\begin{align*}
...
\begin{eg}
	...
	Consider $X = (0,1) \cap \mathbb{Q}$. Enumerate $\mathbb{Q} \subset \left\{ x_{n} \mid n \geq 0 \right\}$. If we let the cover $\mathcal{U} = \left\{ B_{2^{-n-10^{3}}}(x_{n}) \right\}$, then the total "length" of the intervals in this cover will be much less than $1$. But this cover by definition includes all rationals in $(0,1)$, but it doesn't even cover the length of the interval $(0,1)$.
\end{eg}
