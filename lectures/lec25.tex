...
\section{Parametrised submanifolds}
...
\begin{remark}[Uniqueness of the tangent space]
	...
	\]
	Since $D(\psi^{-1}\circ\varphi)(u_0)$ is invertible, the images coincide:
	\[
		\operatorname{Im}(D\varphi(u_0)) = \operatorname{Im}(D\psi(v_0)).
	\]
\end{remark}
...
\section{Local integration theorems}
...
\begin{prop}[Divergence theorem reduced version]\label{prop:div-thm-red}
...
\begin{proof}
	...
	\end{align*}
	But by Fubini, we can integrate for each vertical line (to do this rigorously gets ugly, see notes), so
	\begin{align*}
