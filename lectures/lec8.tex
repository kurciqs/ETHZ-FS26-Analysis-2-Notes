...
\begin{remark}
	Clearly, $f: U \to \mathbb{R}^{m}$ is differentiable at $x_{0} \in U$ if and only if $f(x_{0} + x) - f(x_{0}) - L(x) = \smalO(\left| x \right|)$.
\end{remark}
...
\begin{theorem}[Sufficient condition for differentiability]\label{thm:suff-cond-diff}
	Let $U \subset \mathbb{R}^{n}$ be open and $f: U \to \mathbb{R}^{m}$. If $\partial_{i}f(x_{0})$ exists and is continuous for every $1 \leq i \leq n$, for any $x_{0} \in U$, then $f$ is differentiable at every $x_{0} \in U$. Furthermore,
	\begin{align*}
...
\begin{proof}
	...
	\begin{align*}
		f(x_{0} + x) - f(x_{0}) = \sum_{k=1}^{n} (\partial_{k}f(x_{0})+ \smalO(1))x_{k} = \sum_{k=1}^{n} \partial_{k}f(x_{0})x_{k} + \smalO(\left| x \right|)
	\end{align*}
	since $\left| \smalO(1) \cdot x_{i} \right| \leq \left| x \right|$ as $x \to x_{0}$. And so we define the function
	\begin{align*}
		Df_{x_{0}}(x) = \sum_{k=1}^{n} \partial_{k}f(x_{0})x_{k}
	\end{align*}
	and get
	\begin{align*}
		f(x_{0} + x) - f(x_{0}) - Df_{x_{0}}(x) = \smalO(\left| x \right|),
	\end{align*}
