...
\begin{prop}[Gradient vanishes at extremum]\label{prop:grad-vanish}
...
\end{theorem}
\begin{proof}
	Assume without loss of generality that $f$ is non-negative on the ball $\overline{ B_{r}(x_{0}) }$. By replacing $f(x)$ by $\tilde{f}(x) = f(x) + \left| x-x_{0} \right|^{2}$, we get $\nabla \tilde{f}(x_{0}) = \nabla f(x_{0})$ and so we can assume without loss of generality that the minimum is strict.

    Given $\epsilon > 0$, we will minimise the "penalised function":
	\begin{align*}
		f_{\epsilon}(x) = f(x) + \frac{1}{2\epsilon}\sum_{j=1}^{k} g_{i}^{2}(x)
	\end{align*}
	defined for $x \in \overline{ B_{r}(x_{0}) }$. We will take a sequence $(\epsilon_{\ell})_{\ell}^{\infty}$ that goes to zero, for instance $\epsilon_{\ell} = \frac{1}{\ell}$. For each $\ell \in \mathbb{N}$, let $x_{\ell}$ be the point of minimum of $f_{\epsilon_{\ell}}(x)$ in $\overline{ B_{r}(x_{0}) } $. 
	...
	\end{align*}
	And as $\ell \to \infty$, the RHS gets smaller and smaller, hence in particular for our subsequence $x_{\ell_{m}}$,
	\begin{align*}
	...
	\end{align*}
	But we assumed $x_{0}$ was a strict minimum, which forces $\bar{x} = x_{0}$. And so we found a subsequence $x_{\ell_{m}} \to x_{0}$ that is always the minimum of $f_{\epsilon_{\ell_{m}}}$ and converges to $x_{0}$.

	Use that to define $y_{m} = x_{\ell_{m}}$ and $\tilde{\epsilon}_{m} = \epsilon_{\ell_{m}}$. Since eventually, $y_{m} \in B_{r}(x_{0})$, it is an interior minimiser of $f_{\tilde{\epsilon}_{m}}$, so we have by \ref{prop:grad-vanish}
	\begin{align*}
		0 = \nabla f_{\tilde{\epsilon}_{m}}(y_{m}) \implies 0 = \tilde{\epsilon}_{m} \nabla f_{\tilde{\epsilon}_{m}}(y_{m}) = \tilde{\epsilon}_{m} \nabla f(y_{m}) + \sum_{j=1}^{k} g_{j}(y_{m}) \nabla g_{j}(y_{m})
	\end{align*}
	since $\nabla (g_{j}^{2}) = 2g_{j} \nabla g_{j}$ can be verified easily. But this means, for any $m$, the above vectors are linearly dependent ($\tilde{\epsilon}_{m}$ is a non-trivial factor) i.e. $\exists \lambda^{m} \in \mathbb{R}^{k+1}$ that has (wlog) norm $\left| \lambda^{m} \right| = 1$ and satisfies
	\begin{align*}
		0 = \lambda^{m}_{0} \nabla f(y_{m}) + \sum_{j=1}^{k} \lambda_{j}^{m} \nabla g_{j}(y_{m}).
	\end{align*}
	But $\lambda^{m}$ is a sequence in the unit sphere $\mathbb{S}^{k} \subset \mathbb{R}^{k+1}$, which is closed and bounded, and so compact (\ref{thm:heine-borel}). Hence it has a convergent subsequence with limit $\lambda \in \mathbb{S}^{k}$, and passing the entire above equation to the limit (all functions involved are continuous),
	\begin{align*}
		\lambda_{0} \nabla f(x_{0}) +\sum_{i=1}^{k} \lambda_{i} \nabla g_{i} (x_{0}) = 0, \quad \lambda_{0}^{2} + \dots + \lambda_{k}^{2} = 1.
	\end{align*}
\end{proof}
