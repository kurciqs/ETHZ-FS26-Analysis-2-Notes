...
\begin{definition}[Integral over a compact $d$-submanifold]\label{def:int-over-comp-subm}
	...
	$$
		\begin{aligned}
			\int_M f \, d \text{vol}_d & :=\sum_{\ell=1}^N \int_{M \cap B_{\ell}\left(x_{\ell}\right)} f \eta_{\ell} \, d \text{vol}_d,
		\end{aligned}
	$$
...
Now we can prove the full correct version, for $C^{1}$ domains with \textit{any} isometry $F$. 
...
\begin{proof}
	...
	\begin{align*}
		...
		\begin{cases}
			F\cdot \tilde{\eta} \quad & \text{on } \overline{ \Omega }                          \\
...
\begin{theorem}[Green's theorem in the plane]\label{thm:green-thm}
